...
class SingleProblemComu(ea.Problem): # 继承Problem父类
    ...
    def aimFunc(self, pop): # 目标函数
        ...
        globalVar.set_value(enumVar.nowFitnessCount,nowCount+1)
        lower = globalVar.get_value(enumVar.cellIdMin)
        upper = globalVar.get_value(enumVar.cellIdMax)
...
def singleOPComu(scene):
    ...
    """===============================实例化问题对象==========================="""
    problem = SingleProblemComu(scene) # 生成问题对象
    """=================================种群设置==============================="""
    Encoding = 'RI'       # 编码方式
    NIND = popSize            # 种群规模
...
def conductComuSingleOP():
    ...
    userCount = globalVar.get_value(enumVar.userCount)
    print("conductComuSingleOP userCount {}".format(userCount))
    #index = 0
    ...
    for index in range(slotNum):
        ...
        best_gen,best_call,best_ObjV,best_Engy,best_Phen = singleOPComu(scene)
        ### important update
